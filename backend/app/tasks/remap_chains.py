...
INDUSTRY_CHAIN_MAP = {
    # 电气机械和器材制造业
    "电气机械和器材制造业": {
        "keywords": ["电力", "电气", "电网", "输变电", "配电"],
        "chain": "电力设备",
        "layer": 1
    },
    # 计算机、通信和其他电子设备制造业
    "计算机、通信和其他电子设备制造业": {
        "keywords": ["光模块", "光器件", "光纤", "光缆", "光通信", "CPO", "光收发"],
        "chain": "光模块",
        "layer": 3
    },
    # 软件和信息技术服务业
    "软件和信息技术服务业": {
        "keywords": ["云计算", "云服务", "SaaS", "软件", "IT服务"],
        "chain": "云计算",
        "layer": 3
    },
}
...
BUSINESS_CHAIN_MAP = {
    ...
    "电力设备": {
        ...
        "exclude": ["光", "芯片", "半导体", "AI", "大模型", "机器人", "通信", "电子", "显示"]
    },
    "新能源": {
        "keywords": ["光伏", "风电", "太阳能", "风力发电", "新能源发电", "清洁能源发电"],
        "exclude": ["芯片", "半导体", "AI", "通信", "电子"]
    },
    "储能": {
        "keywords": ["储能系统", "储能电池", "储能设备", "抽水蓄能", "液流电池"],
        "exclude": ["芯片", "半导体", "AI", "光模块", "通信", "电子"]
    },
    "核电": {
        "keywords": ["核电", "核能", "核电站"],
        "exclude": []
    ...
    "GPU/AI芯片": {
        ...
        "exclude": ["光模块", "光器件", "光纤", "通信设备"]
    },
    "CPU": {
        "keywords": ["CPU芯片", "中央处理器", "x86处理器", "ARM处理器"],
        "exclude": ["光模块", "光器件", "通信设备"]
    },
    "存储芯片": {
    ...
    "连接器/线缆": {
        ...
        "exclude": ["芯片", "半导体", "AI", "大模型", "光模块", "光收发", "光器件", "光引擎", "通信设备"]
    },
    ...
    "通信设备": {
        "keywords": ["通信设备制造", "通信系统集成", "通信终端设备", "基站设备制造", "天线设备制造", "射频设备制造", "通信设备研发", "通信系统研发"],
        "exclude": ["芯片", "半导体", "光模块", "显示", "屏幕", "面板", "PCB", "覆铜板", "电子元器件", "电子陶瓷", "存储芯片", "闪存", "激光", "光学", "蓝宝石", "玻璃基板", "液晶", "消费电子", "精密制造", "电子材料", "特种气体", "打印复印", "卫星", "计算机硬件", "LED", "电子器件", "电子设备", "电子零部件", "电子专用设备", "电子材料", "电子陶瓷", "电子元件", "电子信息", "电子制造", "电子行业", "电子产业", "电子产品", "电子系统", "电子组件", "电子配件", "电子模块", "电子单元", "电子部件", "电子零件", "电子构件", "电子元素", "电子要素", "电子成分", "电子部分", "电子片段", "电子环节", "电子节点", "电子连接", "电子接口", "电子端口", "电子插口", "电子插座", "电子接头", "电子接插件", "电子连接器", "电子连接件", "电子连接线", "电子连接头", "电子连接口", "电子连接端", "电子连接点", "电子连接处", "电子连接位", "电子连接位点", "电子连接位置", "电子连接部位", "电子连接地方", "电子连接所在", "电子连接所在点", "电子连接所在处", "电子连接所在位", "电子连接所在位置", "电子连接所在部位", "电子连接所在地方", "电子连接所在所在", "电子连接所在所在点", "电子连接所在所在处", "电子连接所在所在位", "电子连接所在所在位置", "电子连接所在所在部位", "电子连接所在所在地方"]
    },
    "液冷/散热": {
    ...
    for chain_name, config in BUSINESS_CHAIN_MAP.items():
        ...
        for keyword in keywords:
            if keyword.lower() in text:
                ...
                # Layer 3 (AI基础设施) 的光模块相关关键词权重更高
                layer_weight = 1.0
                if chain_name == "光模块" and layer == 3:
                    layer_weight = 1.5  # 光模块权重提高50%

                score = len(keyword) * layer_weight
...
def match_chain_by_industry(name: str, industry: str) -> tuple[str, int] | None:
    """根据行业分类匹配产业链"""
    if not industry:
        return None

    # 查找行业映射
    for ind_name, config in INDUSTRY_CHAIN_MAP.items():
        if ind_name in industry:
            # 检查关键词
            text = f"{name} {industry}".lower()
            for keyword in config["keywords"]:
                if keyword.lower() in text:
                    return config["chain"], config["layer"]
...
async def remap_stock_chains():
    ...
    try:
        ...
        for code, name, industry, core_business in stocks:
            ...
            # 如果没有匹配到，尝试匹配行业
            if not match:
                match = match_chain_by_industry(name, industry or "")
