...
雪球采集器基于akshare库实现，采集三种热度数据：

### 采集器类型
- `XueqiuCollector` - 讨论热度排行（股票在雪球上的讨论数量）
- `XueqiuDealCollector` - 交易热度排行（股票在雪球上的交易分享数量）
- `XueqiuFollowCollector` - 关注热度排行（股票在雪球上的关注人数）

### 数据特点
- 采集频率：30分钟一次
- 数据量：每个采集器约5000-6000条数据
- 数据格式：包含股票代码、名称、热度值、最新价
- 重要度计算：根据热度值动态调整重要度分数
- 数据保留：3天（热度数据更新频繁，历史数据价值较低）
...
### 清理策略
- 普通资讯保留14天
- 雪球热度数据保留3天（因为热度数据是快照，每次采集都会生成新记录）
- 每天凌晨3点执行清理任务
